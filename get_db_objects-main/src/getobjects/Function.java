...
public class Function extends SnowflakeObject {

	Function() {
		setObjectType(SnowflakeObjectType.FUNCTION);
	}
	
	//has the signature if reading form an inventory file
	Function(Connection connection, String project, String targetEnvironment, String parentDb, String targetDb, String parentSchema, String name, String signature) throws Exception {
	...
	public String extractDdl(Connection connection) throws Exception {
		...
		funcDdl = "CREATE FUNCTION " + this.getName() + procMetadata.get("signature") + 					"\n";				
		funcDdl += " RETURNS " + 						procMetadata.get("returns") + 						"\n";
		funcDdl += " LANGUAGE " + 						procMetadata.get("language") + 						"\n";
		
		if (procMetadata.get("null handling") != null ) {
		funcDdl += " " + 								procMetadata.get("null handling") + 				"\n";
		}
		
		if (procMetadata.get("volatility") != null) {
		funcDdl += " " + 								procMetadata.get("volatility") + 					"\n";
		}
		funcDdl += " as $$ " + 							procMetadata.get("body") + 							"$$;";
